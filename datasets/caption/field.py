...
class ImageField:
    ...
    #     except KeyError:
    #         print('Could not find detections for %d' % image_id)
    #         #if use fastercnn (10,2048)
    #         precomp_data = np.random.rand(100,256)
    ...
    #     outputs = {}
    #     outputs['reg_feat'] = torch.from_numpy(precomp_data.astype(np.float32))
    #     reg_mask = torch.from_numpy(np.zeros((self.max_detections), dtype=np.float32))
    #     reg_mask[self.max_detections-delta:]=1
    #     outputs['reg_mask'] = reg_mask
    #     outputs['reg_mask']=  outputs['reg_mask'].unsqueeze(0).unsqueeze(0)

    #     def grid_feat(img_id):
    #         hdf5_path='/data/zys/ubuntu_grit/coco2014/DIFNet_feat/grid/merged_X101_trainval.hdf5'
    #         hdf5_file=h5py.File(hdf5_path,'r')
    #         # print(list(hdf5_file.keys())[0:10])
    #         feat=hdf5_file[f'{img_id}_grids']
    #         return np.array(feat)
    #     outputs['gri_feat']=torch.from_numpy(grid_feat(image_id))#49,2048
    #     outputs['gri_mask']=torch.from_numpy(np.zeros((49), dtype=np.float32))#1 1 49
    #     outputs['gri_mask']=outputs['gri_mask'].unsqueeze(0).unsqueeze(0)
    #     return outputs
    ...
    #                 # print(outputs['reg_feat'].shape)
    #                 # print(outputs['reg_mask'][:100])
    #         return outputs
    #     else:
    #         img = Image.open(path).convert('RGB')
    #         if self.transform is not None:
