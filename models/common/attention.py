...
class MaskHalfAttention(nn.Module):
    '''
    ...
    def forward(self, q, k, v, attention_mask=None, attention_weights=None):
        ...
        #random mask half of the scores in last dim
        mask = torch.rand(scores.shape[-1]).cuda() > 0.9
        mask = mask.unsqueeze(0).unsqueeze(0).repeat(scores.shape[0], scores.shape[1], 1, 1)
...
class Attention(nn.Module):
    ...
    def forward(self, q, k, v, attention_mask=None, attention_weights=None):
        ...
        if attention_mask is not None:
            scores = scores.masked_fill(attention_mask.bool(), -np.inf)
            # scores = scores.masked_fill(attention_mask.bool().unsqueeze(1).unsqueeze(1), -np.inf)
        p_attn = torch.softmax(scores, -1)
        p_attn = self.dropout(p_attn)
