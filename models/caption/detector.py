...
class Detector(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        if self.use_reg_feat:
            ...
            hs, init_reference_out, inter_references_out = self.det_module(srcs, masks)
            outputs['reg_feat'] = hs[-1]
            outputs['reg_mask'] = hs[-1].data.new_full((hs[-1].shape[0], 1, 1, hs[-1].shape[1]), 0).bool()
            outputs['init_reference_out']= init_reference_out
            outputs['inter_references_out']= inter_references_out
        return outputs
